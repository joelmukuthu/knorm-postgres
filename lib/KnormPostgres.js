...
class KnormPostgres {
  ...
  updateSql(knorm) {
    class PostgresCondition extends knorm.Sql.Condition {
      formatType(sql, { column, placeholder }) {
        if (this.type === 'ilike') {
          return { sql: `${column} ILIKE ${placeholder}` };
        }

        return super.formatType(sql, { column, placeholder });
      }
    }

    class PostgresSql extends knorm.Sql {
      ilike(field, value) {
        return new this.constructor.Condition({ type: 'ilike', field, value });
      }
    }

    PostgresSql.updateCondition(PostgresCondition);

    knorm.updateSql(PostgresSql);
  }
